...
def test_build_queue_generates_matching_problems() -> None:
    ...
    queue = build_queue(plan, make_profile("times-tables"), length=20, rng=secrets.SystemRandom(42))
    assert len(queue) == 20
    for item in queue:
...
def test_submit_updates_progress_and_advances() -> None:
    ...
    session = PracticeSession.start(profile, get_plan("everything"), length=3, rng=secrets.SystemRandom(1))
...
def test_session_state_synced_and_resumable() -> None:
    profile = make_profile()
    session = PracticeSession.start(profile, get_plan("everything"), length=4, rng=secrets.SystemRandom(2))
...
def test_finishing_clears_session_state() -> None:
    profile = make_profile()
    session = PracticeSession.start(profile, get_plan("everything"), length=2, rng=secrets.SystemRandom(3))
...
def test_abandon_clears_state_but_keeps_progress() -> None:
    profile = make_profile()
    session = PracticeSession.start(profile, get_plan("everything"), length=3, rng=secrets.SystemRandom(4))
    item = session.current
    assert item is not None
    session.submit(item.problem.answer)
    session.abandon()
...
def test_resume_with_stale_trick_name_returns_none() -> None:
    profile = make_profile()
    _session = PracticeSession.start(profile, get_plan("everything"), length=2, rng=secrets.SystemRandom(5))
    assert profile.session is not None
