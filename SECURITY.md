# Security Policy for `sumsmaster`
...
1. **GitHub private vulnerability reporting** —
   [submit a confidential report](https://github.com/matthewdeanmartin/sumsmaster/security/advisories/new)
...
For maximum isolation, run `sumsmaster` inside a container so it only has access to what you
explicitly mount.
...
```bash
docker build -t sumsmaster .

# Mount the current directory into /data and pass it as an argument
docker run --rm -v "$(pwd):/data" sumsmaster /data
```
